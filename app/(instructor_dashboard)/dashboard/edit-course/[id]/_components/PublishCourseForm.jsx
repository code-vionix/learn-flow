...
const PublishCourseForm = ({ course }) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      price: course?.price || 0, // Default value for price
      discount: course?.discountPercentage || 0, // Default value for discount
    },
  });
  useEffect(() => {
    ...
    setValue("discount", course?.discountPercentage);
    if (data && course?.CourseInstructor?.length > 0) {
      setValue(
        "instructors",
        course?.CourseInstructor?.map((instructor) => instructor.instructorId)
      );
