...
import { Plus, Edit, Trash2, ChevronUp, ChevronDown, VideoIcon, File } from "lucide-react";
            ...
            <div className="flex items-center justify-between p-3">
              <div className="flex items-center gap-2 flex-1">
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  {lecture.description && (
                    <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full">Description</span>
                  )}
                  {lecture.notes && (
              ...
              <div className="flex items-center gap-2">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    ...
                    >
                      Contents {lecture.isOpen ? <ChevronUp className="ml-1 h-4 w-4" /> : <ChevronDown className="ml-1 h-4 w-4" />}
                    </Button>
                  </DropdownMenuTrigger>
                  {lecture.isOpen && (
                    <DropdownMenuContent align="end" className="w-[180px]">
                      <DropdownMenuItem onClick={() => openVideoUploadDialog(section.id, lecture.id)}>
                        <VideoIcon className="mr-2 h-4 w-4" /> <span>Video</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => openAttachFileDialog(section.id, lecture.id)}>
                        <File className="mr-2 h-4 w-4" /> <span>Attach File</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => openCaptionDialog(section.id, lecture.id)}>
                        <span className="ml-6">Captions</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => openDescriptionDialog(section.id, lecture.id)}>
                        <span className="ml-6">Description</span>
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => openNotesDialog(section.id, lecture.id)}>
