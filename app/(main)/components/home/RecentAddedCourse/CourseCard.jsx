...
const CourseCard = ({ course }) => {
  ...
  return (
    ...
    >
          ...
          <div className="border-t border-gray-200 mt-2">
            <div className="px-4 py-3 flex justify-between items-center">
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm text-gray-700">{reviews.length}</span>
              </div>
              <div className="flex items-center gap-1 px-4">
                <User className="w-5 h-5 text-[#564FFD]" />
                <span className="text-sm text-gray-700">
                  {(course?.enrollments?.length / 10).toFixed(1)} students
                </span>
              </div>
            </div>
          </div>
      ...
      {isHovered && (
        <div
          className={`absolute top-1/2 -translate-y-[50%] z-30 ${
            showLeft ? "right-full -mr-4" : "left-full -ml-4"
          }`}
        >
          <CourseHoverCard
            course={course}
            position={showLeft ? "left" : "right"}
          />
        </div>
      )}
    </div>
